...
data_summarized <- total_cogs_tf %>%
  select(organism, ORFs, phylum, class) %>%
  bind_cols(Transcription_Factor = total_cogs_tf$total) %>%
  bind_cols(Sigma_Factor = total_cogs_sigma$total) %>%
...
for (i in selected_phylogeny) {
  filename = paste0("figures/sup1_",i, ".tiff")
